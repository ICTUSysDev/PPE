var app = angular.module('machineryEquipmentPar',['account-module','app-module']);

app.controller('machineryEquipmentParCtrl',function($scope,app) {
...
app.filter('paginationICS', function() {
	return function(input, currentPageICS, pageSizeICS) {
		if(angular.isArray(input)) {
			var start = (currentPageICS-1)*pageSizeICS;
			var end = currentPageICS*pageSizeICS;
			return input.slice(start, end);
		}
	};
});