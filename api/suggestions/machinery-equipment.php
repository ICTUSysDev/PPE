...
$active_tab = $_POST['tabId'];

if($active_tab == 'icsList') {
  ...
  $machinery_equipments = $con->getData("SELECT id, property_number, article_id FROM machinery_equipment WHERE status = 'Available' AND equipment_condition = 'SERVICEABLE' AND acquisition_cost >= 50000");
  ...
  $furniture_and_fixtures = $con->getData("SELECT id, property_number, article_id FROM furniture_and_fixtures WHERE status = 'Available' AND carrying_amount >= 50000");
