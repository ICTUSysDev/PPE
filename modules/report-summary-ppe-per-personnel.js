angular.module('app-module',['notify', 'my-pagination','ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui','notify','form-validator', 'module-access']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,validate,bui,myPagination,notify, access) {
    ...
    var self = this;
		...
		self.data = function(scope) {
			...
			scope.headerName = "Machinery and Equipment - Summary of PPE per Personnel";
    ...
    // List Start
		self.list = function(scope) {
			...
			scope.controls.edit.label = "Edit";
				...
				$http({
					method: 'POST',
					url: 'handlers/report-summary-ppe-per-personnel/list.php',
				...
				$('#content').load('lists/report-summary-ppe-per-personnel.html', function() {
		...
		self.viewReport=function(scope,row){
			...
			$http({
				method: 'POST',
				url: 'handlers/report-summary-ppe-per-personnel/viewReport.php',
				data: {employee_id: row.id}

			}).then(function mySucces(response) {
				scope.datas = response.data;

				if (scope.datas.list_of_machineries === undefined)
				{
					growl.show('alert alert-danger',{from: 'top', amount: 55},'No equipments issued to ' + scope.datas.employee_name);
					return;
				}
				else
				{
			...
			// error
			growl.show('alert alert-danger',{from: 'top', amount: 55},response);
			});
		};
      ...
      function print(scope) {
			...
			doc.text(165, 30, "As of " + scope.date_today, 'center');
			
			doc.setFontSize(9);
			doc.setFont('helvetica','bold','bold');
			doc.text(50, 41, scope.employee_name,{ align: 'center', maxWidth:50});
			doc.text(105, 41, scope.position_description, { align: 'center', maxWidth:75});
			doc.text(185, 41, scope.office_name, 'center');
			...
			angular.forEach(scope.list_of_machineries, function(data,key) {
			
				var row = [];
				row.push(data.article);
				...
				if (scope.list_of_machineries.length == (parseInt(key)+1))
				{
					var row1 = [];
					rows.push(row1);
				...
				row1.unshift({
					colSpan: 7,
					content: 'Total: ' + getFormattedAmount(scope.total_acquisition_cost),
			...
			doc.setFontSize(6);
			doc.text(12.7, 210, 'Report generated by ' + scope['userFullName'] + ' at ' + scope['reportDate'],'left');
