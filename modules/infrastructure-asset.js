angular.module('app-module',['notify', 'my-pagination','ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui','notify','form-validator', 'module-access']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,validate,bui,myPagination,notify, access) {
    ...
    var self = this;
		...
		self.data = function(scope) {
			...
			scope.headerName = "Infrastructure Asset";

			// for Validation
			scope.formHolder = {};

			// CRUD
			scope.infrastructureAsset = {}
			scope.infrastructureAsset.id = 0;

			// List
			scope.infrastructureAssets = [];
    ...
    // List Start
		self.list = function(scope) {
			...
			scope.controls.edit.label = "Edit";
				...
				$http({
					method: 'POST',
					url: 'handlers/infrastructure-assets/list.php',
					data: scope.infrastructureAssets
				}).then(function mySucces(response) {
					
					scope.infrastructureAssets = angular.copy(response.data);
					
				}, function myError(response) {

				});

				$('#content').load('lists/infrastructure-assets.html', function() {
		...
		// Form Start
		self.infrastructureAsset = function(scope,row) {
			...
			$('#content').load('forms/infrastructure-asset.html',function() {
			...
			if (row != null) {
				...
				$http({
				  method: 'POST',
				  url: 'handlers/infrastructure-assets/view.php',
				  data: {id: row.id}
				}).then(function mySucces(response) {
					
					angular.copy(response.data, scope.infrastructureAsset);

					scope.filteredBarangays = response.data.municipality_id.barangays;

					if (response.data.date_added != null){scope.infrastructureAsset.date_added = new Date(response.data.date_added);}else{};

					self.fileList(scope,scope.infrastructureAsset.id);
		...
		self.save = function(scope) {
			...
			if (validate.form(scope,'infrastructureAsset')){ 
			...
			var onOk =  function() {
				...
				$http({
					method: 'POST',
					url: 'handlers/infrastructure-assets/save.php',
					data: {infrastructureAsset: scope.infrastructureAsset}
				}).then(function mySuccess(response) {

					if(scope.infrastructureAsset.id==0){
						scope.infrastructureAsset.id = response.data;
					}
		...
		self.delete = function(scope,row) {
			...
			var onOk = function() {
				...
				$http({
					method: 'POST',
					url: 'handlers/infrastructure-assets/delete.php',
					data: {id: [row.id]}
				}).then(function mySucces(response) {
		...
		function coaCodes(scope) {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/infrastructure-assets/charts-of-accounts.php',
			}).then(function mySucces(response) {
		...
		self.filterBarangay = function(scope,item) {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/infrastructure-assets/filter-barangays.php',
		...
		self.openMap = function(scope) {
			...
			if ((typeof scope.infrastructureAsset.latitude != 'undefined' ) || (typeof scope.infrastructureAsset.longtitude != 'undefined')) {
				lat = scope.infrastructureAsset.latitude;
				lng = scope.infrastructureAsset.longtitude;
			}
				...
				var onOk = function(scope){
					if(marker == null){
						...
						scope.infrastructureAsset.latitude = coordinates.lat;
						scope.infrastructureAsset.longtitude = coordinates.lng
					}
      ...
      $('#uploadFile').click(function() {
				...
				$.ajax({
						url: 'handlers/infrastructure-assets/upload-files.php',
						type: 'POST',
						data: formData,
						dataType: 'json',
						processData: false,
						...
						success: function(response) {
							...
							scope.infrastructureAsset.fileNames = response;
				...
				setTimeout(function() {
					...
					var moduleName = 'INFRASTRUCTURE ASSET';
					...
					var onOk =  function() {
						...
						$http({
							...
							data: {fileNames: scope.infrastructureAsset.fileNames, landId: scope.infrastructureAsset.id, module_name: moduleName}
						}).then(function mySucces(response) {
		...
		self.downloadFile = function(scope, itemId) {
			...
			var fileUrl = './assets/files/infrastructure-assets/';
