angular.module('app-module',['my-pagination','ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui','notify','form-validator', 'module-access']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,validate,bui,myPagination,notify, access) {
    ...
    var self = this;
		...
		self.data = function(scope) {
			...
			scope.headerName = "Physical Inventory List";
    ...
    // List Start
		self.list = function(scope) {
			...
			scope.showEditButton = false;
				...
				$http({
					...
					data: scope.physicalInventoryList
				}).then(function mySucces(response) {
					
					scope.physicalInventoryList = angular.copy(response.data);

					self.filterPhysicalInventory(scope);
		...
		self.filterPhysicalInventory = function(scope) {
			
			$http({
			  method: 'POST',
			  url: 'handlers/physical-inventory-list/search-physical-inventory.php',
			  data: scope.filter
			}).then(function mySucces(response) {

				scope.physicalInventoryList = response.data;
		...
		function inventoryBy(scope) {
	
			$http({
				method: 'POST',
				url: 'handlers/physical-inventory-list/suggestions/inventory-by-list.php'
			}).then(function mySuccess(response) {
				
				scope.inventoryBy = angular.copy(response.data);
		...
		self.viewPhysicalInventoryReport = function(scope) {
			...
			$http({
			  method: 'POST',
			  url: 'handlers/physical-inventory-list/report/physical-inventory-report.php',
			  data: scope.filter
			}).then(function mySucces(response) {

				scope.viewPhysicalInventoryReport = response.data;
				print(scope);
		...
		function print(scope) {
			...
			doc.setFont('helvetica','bold','bold');
			doc.text(165, 30, scope.viewPhysicalInventoryReport.physical_inventory.inventory_module, 'center');
			...
			doc.setFont('helvetica','bold','bold');
			doc.text(165, 41, scope.viewPhysicalInventoryReport.physical_inventory.as_of_date, 'center');
			doc.line(146, 42, 194, 42);

			doc.setFontSize(8);
			doc.setFont('helvetica','normal','normal');
			doc.text(12.7, 49, 'Inventory By: ');
			doc.setFont('helvetica','bold','bold');
			doc.text(30, 49, scope.viewPhysicalInventoryReport.physical_inventory.inventory_by_name);
			doc.line(28, 50, 69 ,50);

			var rows = [];

			angular.forEach(scope.viewPhysicalInventoryReport, function(data,key) {
			...
			doc.setFontSize(6);
			doc.text(12.7, 210, 'Report generated by ' + scope.viewPhysicalInventoryReport.reportDetail['userFullName'] + ' at ' + scope.viewPhysicalInventoryReport.reportDetail['reportDate'],'left');
