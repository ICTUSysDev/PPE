angular.module('app-module',['my-pagination','ui.bootstrap','bootstrap-modal','bootstrap-growl','block-ui','notify','form-validator', 'module-access']).factory('app', function($compile,$timeout,$http,bootstrapModal,growl,validate,bui,myPagination,notify, access) {
    ...
    var self = this;
		...
		self.data = function(scope) {
			...
			scope.headerName = "Pre-Repair Inspection";
			...
			// CRUD
			scope.repairOffice = {}
			scope.repairOffice.id = 0;

			// CRUD
			scope.wasteMaterial = {}
			scope.wasteMaterial.id = 0;

			// List
			scope.repairOffices = [];
			scope.wasteMaterials = [];
			...
			// for Pagination
			scope.viewsRL = {};
			scope.viewsRL.currentPageRL = 1;
			scope.viewsRL.list = true;
    ...
    // List Start
		self.list = function(scope) {
			...
			scope.wasteMaterial.quantity = "";
			scope.wasteMaterial.unit = "";
			scope.wasteMaterial.description = "";
			scope.wasteMaterial.receipt_number = "";
			scope.wasteMaterial.amount = "";

				if (scope.$id > 2) scope = scope.$parent;

				$http({
					method: 'POST',
					url: 'handlers/repair-office/list.php',
					data: scope.repairOffices
				}).then(function mySucces(response) {
					
					scope.repairOffices = angular.copy(response.data);
					
					self.requestList(scope);

				}, function myError(response) {

				});

				$('#content').load('lists/repair-office.html', function() {
					$timeout(function() { $compile($('#content')[0])(scope); },100);								
				});
		};

		self.requestList = function(scope) {
			...
			scope.repairForm = {};
				...
				$http({
					method: 'POST',
					url: 'handlers/repair-office/request-repair/list.php',
					data: scope.repairs
				}).then(function mySucces(response) {
		...
		self.repair = function(scope,row) {
			...
			if (row != null) {
				...
				$http({
				  method: 'POST',
				  url: 'handlers/repair-office/request-repair/view.php',
				  data: {ppe_data: row}
				}).then(function mySucces(response) {
		...
		self.approvedRequest = function(scope, equipmentId) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$http({
						method: 'POST',
						url: 'handlers/repair-office/request-repair/repair-form/save.php',
						data: {approvedId: equipmentId, par_id: scope.repairs[0]}
					}).then(function mySuccess(response) {

						Swal.fire({
							title: "Approved Successfully!",
							text: "Do you want to redirect to the Pre-Repair Inspection Form?",
							icon: "success",
							showCancelButton: true,
							confirmButtonColor: "#3085d6",
							...
							if (result.isConfirmed) {
								...
								self.viewForm(scope, response.data);
		...
		self.releaseEquipment = function(scope, equipmentId) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$http({
						method: 'POST',
						url: 'handlers/repair-office/request-repair/release-repair/save.php',
		...
		// Form Start
		self.repairOffice = function(scope,row) {
			...
			scope.wasterMaterials = [];

			scope.showAddButton = false;

			mode(scope, row);
			
			$('#content').load('forms/repair-office.html',function() {
				$timeout(function() { $compile($('#content')[0])(scope); },200);
			});
			...
			if (row != null) {
				...
				$http({
				  method: 'POST',
				  url: 'handlers/repair-office/view.php',
				  data: {id: row.id}
				}).then(function mySucces(response) {
					
					angular.copy(response.data, scope.repairOffice);

					mode(scope, row);

					if (response.data.date_started != null){scope.repairOffice.date_started = new Date(response.data.date_started);}else{};
					if (response.data.date_finish != null){scope.repairOffice.date_finish = new Date(response.data.date_finish);}else{};
					if (response.data.date_released != null){scope.repairOffice.date_released = new Date(response.data.date_released);}else{};

					self.machineryEquipmentList(scope);
					
					self.fileList(scope,scope.repairOffice.id);
					self.listwasteMaterial(scope);
					self.serialNumber(scope,scope.repairOffice.equipment_id);
			...
			// machineryEquipmentList(scope);
			fileUpload(scope);

		};

		self.viewForm = function(scope,row) {
		...
		self.save = function(scope) {
			...
			if (validate.form(scope,'repairOffice')){ 
			...
			var spinner;
				...
				$http({
					method: 'POST',
					url: 'handlers/repair-office/save.php',
					data: {repairOffice: scope.repairOffice}
				}).then(function mySuccess(response) {
					...
					setTimeout(function() {
							...
							if(scope.repairOffice.id==0){
									scope.repairOffice.id = response.data;
							} else {
							...
							Swal.fire({
								...
								html: `Do you want to print Report or Add Waste Material?`,
								icon: "success",
								showDenyButton: true,
								showCancelButton: true,
								confirmButtonText: "Print Report",
								denyButtonText: `Waste Material`,
								cancelButtonText: `No thanks!`
							}).then((result) => {
								/* Read more about isConfirmed, isDenied below */
								if (result.isConfirmed) {
									self.viewRequestForm(scope);
									self.list(scope);
								} else if (result.isDenied) {
									document.querySelector('a[data-kt-countup-tabs="true"][href="#waste_material"]').click();
		...
		self.delete = function(scope,row) {
			...
			var onOk = function() {
				...
				$http({
					method: 'POST',
					url: 'handlers/repair-office/delete.php',
					data: {id: [row.id]}
				}).then(function mySucces(response) {
		...
		self.viewRequestForm = function(scope) {

			$http({
				method: 'POST',
				url: 'handlers/repair-office/reports/view-request-form.php',
				data: {repairOffice: scope.repairOffice}
			}).then(function mySuccess(response) {
		...
		self.machineryEquipmentList = function(scope) {
	
			$http({
				method: 'POST',
				url: 'handlers/repair-office/repair-form/machinery-equipment-list.php',
				data: {propertyType: scope.repairOffice.equipment_description}
			}).then(function mySuccess(response) {
				
				scope.machineryEquipmentList = angular.copy(response.data);
		...
		self.serialNumber = function(scope, row) {

			$http({
				method: 'POST',
				url: 'handlers/repair-office/repair-form/serial-number.php',
				data: {id: row}
			}).then(function mySucces(response) {
				
				scope.serialNumber = angular.copy(response.data);
      ...
      $('#uploadFile').click(function() {
				...
				if(scope.repairOffice.id == 0) {
					growl.show('alert alert-danger',{from: 'top', amount: 55},'Please Information first');
						return;
				}
				...
				$.ajax({
						url: 'handlers/repair-office/upload-files.php',
						type: 'POST',
						data: formData,
						dataType: 'json',
						processData: false,
						...
						success: function(response) {
							...
							scope.repairOffice.fileNames = response;
				...
				setTimeout(function() {
					...
					var moduleName = 'REPAIR OFFICE';
				
					$http({
						method: 'POST',
						url: 'handlers/file-uploads/save-file-names.php',
						data: {fileNames: scope.repairOffice.fileNames, landId: scope.repairOffice.id, module_name: moduleName}
					}).then(function mySucces(response) {
		...
		self.downloadFile = function(scope, itemId) {
			...
			var fileUrl = './assets/files/repair-office/';
		...
		// Form Start
		self.wasteMaterial = function(scope,row) {
			...
			if (row != null) {
				...
				$http({
				  method: 'POST',
				  url: 'handlers/repair-office/waste-material/view.php',
				  data: {id: row.id}
				}).then(function mySucces(response) {
					
					angular.copy(response.data, scope.wasteMaterial);
		...
		// Add Function Start
		self.saveWasteMaterial = function(scope) {
			
			if (validate.form(scope,'wasteMaterial')){ 
				growl.show('alert alert-danger',{from: 'top', amount: 55},'Some fields are required.');
				return;
			}

			if(scope.repairOffice.id==0) {
				growl.show('alert alert-danger',{from: 'top', amount: 55},'Please add or save Pre-Repair Inspection first!');
				return;
			}
		
				$http({
					method: 'POST',
					url: 'handlers/repair-office/waste-material/save.php',
					data: {wasteMaterial: scope.wasteMaterial, repairOfficeId: scope.repairOffice.id, repairEquipmentId: scope.repairOffice.machinery_equipment.id}
				}).then(function mySuccess(response) {
					...
					setTimeout(function() {
							...
							if(scope.wasteMaterial.id==0){
									scope.wasteMaterial.id = response.data;
									self.listwasteMaterial(scope);
							} else {
									self.listwasteMaterial(scope);
							};
							...
							Swal.fire({
								title: "Waste Material Added Successfully!",
								icon: "success",
								showDenyButton: true,
								showCancelButton: true,
    ...
    // List Start
		self.listwasteMaterial = function(scope) {
			...
			scope.showEditButton = true;
				...
				$http({
					method: 'POST',
					url: 'handlers/repair-office/waste-material/list.php',
					data: {repairOfficeId: scope.repairOffice.id}
				}).then(function mySucces(response) {
					
					scope.wasteMaterials = angular.copy(response.data);
		...
		// Delete Start
		self.deletewasteMaterial = function(scope,row) {
			...
			var onOk = function() {
				...
				$http({
					method: 'POST',
					url: 'handlers/repair-office/waste-material/delete.php',
					data: {id: [row.id]}
				}).then(function mySucces(response) {
		...
		self.viewWasteMaterial = function(scope) {

			$http({
				method: 'POST',
				url: 'handlers/repair-office/reports/view-waste-material.php',
				data: {id: scope.repairOffice.id}
			}).then(function mySuccess(response) {
		...
		doc.setFontSize(10);
		doc.setFont('helvetica','normal');
		doc.text('Administrative Officer III', 157, lastRowY+20);
		...
		function preRepairInspection(scope) {
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.property_number==null?'':scope.preRepairInspection.property_number.description, 43.8, 34)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.property_number.brand_id==null?'N/A':scope.preRepairInspection.property_number.brand.name, 43.8, 38)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(`${scope.preRepairInspection.serial_number==null?'':scope.preRepairInspection.serial_number}`, 43.8, 41.8)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.property_number==null?'':scope.preRepairInspection.property_number.property_number, 152.1, 38)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.acquisition_cost==null?'':scope.preRepairInspection.acquisition_cost, 152.1, 41.8)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			const natureOfWorkText = scope.preRepairInspection.nature_of_work==null?'':scope.preRepairInspection.nature_of_work;
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.complaints==null?'' : scope.preRepairInspection.complaints, 152.1, 51.1)
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			doc.text(scope.preRepairInspection.repair_history==null?'':scope.preRepairInspection.repair_history.repair_date, 152.1, 55.5)
			...
			// Example usage
			const requestedBy = scope.preRepairInspection.requested_by.requested_by_user==null?'':scope.preRepairInspection.requested_by.requested_by_user.requested_by_user;
			...
			// Example usage
			const NotedBy = scope.preRepairInspection.noted_by.signatories==null?'':scope.preRepairInspection.noted_by.signatories;
			const NotedByX = 155;
			const NotedByY = 81.5;
			...
			doc.setFontSize(10);
			doc.setFont('helvetica','normal');
			const longText = scope.preRepairInspection.pre_findings==null?'':scope.preRepairInspection.pre_findings;
			...
			// Example usage
			const checkedBy1 = scope.preRepairInspection.checked_by[0].signatories==null?'':scope.preRepairInspection.checked_by[0].signatories;
			...
			// Example usage
			const checkedBy2 = scope.preRepairInspection.checked_by[2].signatories==null?'':scope.preRepairInspection.checked_by[2].signatories;
			...
			// Example usage
			const checkedBy3 = scope.preRepairInspection.checked_by[3].signatories==null?'':scope.preRepairInspection.checked_by[3].signatories;
			...
			// Example usage
			const checkedBy4 = scope.preRepairInspection.checked_by[4].signatories==null?'':scope.preRepairInspection.checked_by[4].signatories;
			...
			// Example usage
			const checkedBy5 = scope.preRepairInspection.checked_by[1].signatories==null?'':scope.preRepairInspection.checked_by[1].signatories;
			...
			const checkedBy = scope.preRepairInspection.checked_by[0].signatories==null?'':scope.preRepairInspection.checked_by[0].signatories;
			const checkedByX = 51;
			...
			doc.setFontSize(9);
			doc.setFont('helvetica','bold');
			doc.text(scope.preRepairInspection.checked_by[0].signatories==null?'':scope.preRepairInspection.checked_by[0].signatories, 134, 188);
			...
			const exec = scope.preRepairInspection.checked_by[2].signatories==null?'':scope.preRepairInspection.checked_by[2].signatories;
			const execX = 47;
			const execY = 199.3;
			const execSize = 9;
			...
			const tres = scope.preRepairInspection.checked_by[3].signatories==null?'':scope.preRepairInspection.checked_by[3].signatories;
			const tresX = 89;
			const tresY = 199.3;
			const tresSize = 9;
			...
			const Acc = scope.preRepairInspection.checked_by[4].signatories==null?'':scope.preRepairInspection.checked_by[4].signatories;
			const AccX = 129;
			const AccY = 199.3;
			const AccSize = 9;
			const AccDistance = .5;
			...
			const gso = scope.preRepairInspection.checked_by[1].signatories==null?'':scope.preRepairInspection.checked_by[1].signatories;
			const gsoX = 170;
			const gsoY = 199.3;
			const gsoSize = 9;
			const gsoDistance = .5;
			...
			const end = scope.preRepairInspection.requested_by.requested_by_user==null?'':scope.preRepairInspection.requested_by.requested_by_user.requested_by_user;
			const endX = 172;
			const endY = 222.5;
			const endSize = 10;
			const endDistance = .5;
