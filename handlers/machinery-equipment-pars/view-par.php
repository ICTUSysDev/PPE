...
foreach($machinery_equipments as $key => $machinery_equipment) {
  ...
  if($machinery_equipmentss[0]['brand_id'] != null) {
    ...
    $machinery_equipments[$key]['par_machinery_equipment']['brand'] = array(
      'id' => 0,
      'name' => 'N/A'
    );
  }
...
foreach($furniture_and_fixtures as $key => $furniture_and_fixture) {
  ...
  $furniture_and_fixturess = $con->getData("SELECT id, property_number, article_id, furniture_and_fixture_condition, CONCAT(carrying_amount) as acquisition_cost, acquisition_date, remarks FROM furniture_and_fixtures WHERE id = ".$furniture_and_fixture['equipment_id']);
