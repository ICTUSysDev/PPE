...
$machinery_equipment_data_return = $_POST['machineryEquipmentData'];
...
foreach ($machinery_equipment_data_return as $medn) {
		...
		if($purpose == 'DISPOSAL') {
			...
			if($medn['equipment_description'] == 'Machinery Equipment') {
				$sql = "UPDATE machinery_equipment SET equipment_condition = 'UNSERVICEABLE', status = 'DISPOSED' WHERE id = $medn[equipment_id]";
				$con->query($sql);
			} else if($medn['equipment_description'] == 'Furniture and Fixture'){
				$sql = "UPDATE furniture_and_fixtures SET equipment_condition = 'UNSERVICEABLE', status = 'DISPOSED' WHERE id = $medn[equipment_id]";
				$con->query($sql);
			}
