...
$machinery_equipment_repars = $con->getData("SELECT id, par_no, accountable_officer, office_id, par_date FROM machinery_equipment_pars WHERE form_type NOT IN('RETURNED', 'ICS', 'DONATION')");
...
foreach($machinery_equipment_repars as $key => $machinery_equipment_repar) {
  ...
  if($machinery_equipment_repar['accountable_officer'] != 0){

    $accountable_officer = $con->getData("SELECT id, CONCAT(first_name,' ',last_name) name, office_id, employee_id FROM users WHERE id = ".$machinery_equipment_repar['accountable_officer']);
