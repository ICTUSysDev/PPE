...
$relocated_equipment = $con->getData("SELECT p.id, p.par_id, p.equipment_id, p.status, p.accountable_officer, m.id, m.ptr_no, m.beneficiary, m.par_date FROM par_machinery_equipment AS p, machinery_equipment_pars AS m WHERE status = 'RELOCATE' AND m.par_date BETWEEN '$start_date' AND '$end_date' AND m.id = p.par_id");

foreach($relocated_equipment as $key => $mep) {
    
  $machinery_equipment = $con->getData("SELECT id, article_id, acquisition_cost, property_number FROM machinery_equipment WHERE id = ".$mep['equipment_id']);
  
    $article_id = $con->getData("SELECT id, name FROM articles WHERE id = ".$machinery_equipment[0]['article_id']);
    $relocated_equipment[$key]['article_id'] = $article_id[0];
    
  $relocated_equipment[$key]['relocated_equipment'] = $machinery_equipment[0];
  
  $par_details = $con->getData("SELECT id, ptr_no, beneficiary, par_date FROM machinery_equipment_pars WHERE id = ".$mep['par_id']);
  $relocated_equipment[$key]['par_details'] = $par_details[0];
  ...
  $relocated_equipment[$key]['accountable_officer'] = $accountable_officer[0];

  $relocated_equipment[$key]['list_no'] = $key+1;
}
...
$transfer_log = array(
  'donated_equipment' => $donated_equipment,
  'relocated_equipment' => $relocated_equipment,
  'reassigned_equipment' => $reassigned_equipment
);
