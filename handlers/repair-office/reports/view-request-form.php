...
$serial_number = isset($repair_office['serial_number']) && is_array($repair_office['serial_number'])? ($repair_office['serial_number']['serial_number'] ?? ''): '';

$pre_repair_inspection = $con->getData("SELECT *, DATE_FORMAT(date_started, '%m/%d/%Y') AS date_started, DATE_FORMAT(date_finish, '%m/%d/%Y') AS date_finish, DATE_FORMAT(date_released, '%m/%d/%Y') AS date_released FROM pre_repair_inspection WHERE id = $repair_office[id]");

// $repair_history = $con->getData("SELECT id, equipment_id, DATE_FORMAT(repair_date, '%M %d, %Y') AS repair_date FROM `repair_history` WHERE equipment_id = ".$pre_repair_inspection[0]['equipment_id']." ORDER BY id DESC LIMIT 1");
...
if($requested_by[0]['accountable_officer'] != null) {
