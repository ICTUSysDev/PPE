...
$repair_equipment = $con->getData("SELECT p.id, p.par_id, p.equipment_id, p.status, p.accountable_officer, m.id, m.par_no, m.beneficiary, m.par_date FROM par_machinery_equipment AS p, machinery_equipment_pars AS m WHERE status = 'RETURN TO STOCK' AND m.par_date BETWEEN '$start_date' AND '$end_date' AND m.id = p.par_id");
...
echo json_encode($repair_equipment);
