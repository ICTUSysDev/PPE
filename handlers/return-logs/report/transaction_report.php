...
foreach($return_logs as $key => $rl) {
  ...
  $par_machinery_equipment = $con->getData("SELECT id, par_id, equipment_id, prev_accountable_officer, status FROM par_machinery_equipment WHERE par_id = ".$rl['id']);
  ...
  foreach($par_machinery_equipment as $i => $pme) {
    ...
    } else if($par_machinery_equipment[0]['status'] == 'REPAIRED') {
