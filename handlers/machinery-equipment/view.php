...
if(!empty($machinery_equipment[0]['location'])) {
  $location = $con->getData("SELECT id, building_and_structure_id, building_and_structure_location FROM building_and_structures WHERE id = ".$machinery_equipment[0]['location']);
  $machinery_equipment[0]['location'] = $location[0];
}
