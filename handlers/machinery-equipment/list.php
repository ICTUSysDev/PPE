...
if($check_user[0]['groups'] == 1 || $check_user[0]['groups'] == 2) {
  $par_machinery_equipment = $con->getData("SELECT id, coa_description_id, property_number, article_id, brand_id, status, useful_life FROM machinery_equipment WHERE status NOT IN('DONATED') AND acquisition_cost >=50000 ORDER BY id DESC");
  ...
  $par_machinery_equipment = $con->getData("SELECT id, equipment_id, office_id FROM par_machinery_equipment WHERE status = 'PAR' AND accountable_officer = ".$check_user[0]['id']." AND office_id = " .$check_user[0]['office_id']);

  foreach($par_machinery_equipment as $index => $pme) {

    $machinery_equipmentss = $con->getData("SELECT id, coa_description_id, property_number, article_id, brand_id, status, useful_life FROM machinery_equipment WHERE id = ".$pme['equipment_id']." AND status NOT IN('DONATED') AND acquisition_cost >=50000 ORDER BY id DESC");
  ...
  $par_machinery_equipment = $con->getData("SELECT id, equipment_id, office_id FROM par_machinery_equipment WHERE status = 'PAR' AND office_id = " .$check_user[0]['office_id']);
